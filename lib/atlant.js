...
function Atlant() {
    ...
    atlantState.rootStream.onValue(s.tryD(function (upstream) {
        ...
        _whens.items.forEach(function (whenData) {
            ...
            if (!stream || !(stream instanceof _incStream2['default'])) throw new Error('Unknown return from Stream function');
    ...
    this.stream = function (bus) {
        return new _incStream2['default'](atlantState, prefs, bus);
    };

    this.streams = {
        _getStream: function _getStream(stream) {
        ...
        get: function get(name) {
            if (!atlantState.streams[name]) atlantState.streams[name] = _this.stream();
...
var Stream = function Stream(atlantState, prefs, bus) {
    if (bus && bus instanceof Bacon.Bus) _utilsLog2['default'].warn('Using Bacon.Bus streams are deprecated');
    ...
    var _send = function _send(stream) {
