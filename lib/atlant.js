...
/***/ function(module, exports, __webpack_require__) {
	...
	var _uniq = __webpack_require__(30);
	...
	var _iterables = __webpack_require__(33);
	...
	var simpleRender = __webpack_require__(41);
	...
	function Atlant() {
	  ...
	  var _register = function _register(promises) {
	    if (!atlantState.register) {
	      _promise2.default.all(promises).then(function (_) {
	        return _log.Console.log.call(_log.server, 'Tricky promises resolved.'), _;
	      }).then(function (_) {
	        return atlantState.devStreams.renderEndStream.push({ trickyData: _ });
	      });
	    } else {
	      _log.Console.error.call(_log.error, 'Tricky promises already registered!');
	    }
	...
	function AtlantStreamConstructor(name, atlantState, prefs) {
	  ...
	  var resolveStatus = function resolveStatus(scope) {
	    if (!streamState.resolved && streamState.resolveWhen && streamState.resolveWhen(scope)) {
	      ...
	      var passScope = streamState.trickyRoute ? { tricky: true } : scope;
	  ...
	  var _trickyRoute = function _trickyRoute() {
