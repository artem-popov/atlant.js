...
function Atlant() {
    ...
    atlantState.rootStream.onValue(s.tryD(function (upstream) {
        ...
        _whens.items.forEach(function (whenData) {
            ...
            var stream = whenData.route.fn instanceof _incStream.Stream ? whenData.route.fn : whenData.route.fn(); // @TODO should be a Stream.

            if (stream instanceof _incStream.Stream) {
                _utilsLog2['default'].warn('Failed stream source:', whenData.route.fn);throw new Error('You should end the Stream. Try add more .end()\'s ');
            };
            if (!stream || !(stream instanceof _incStream.ReadyStream) && !(stream instanceof Bacon.Bus)) {
                _utilsLog2['default'].warn('Failed stream source:', whenData.route.fn);throw new Error('Unknown return from Stream function, should be atlant.stream or Bacon.Bus', whenData.route.fn);
            };
    ...
    var _when = (function () {
        ...
        return function (masks, fn, matchingBehaviour, whenType) {
            ...
            if ('function' !== typeof fn) {
                _utilsLog2['default'].warn('Failed stream source:', fn);throw new Error('Make use "fn = _ => Stream" as second parameter of atlant.when() for ' + masks);
            };
            ...
            name = name + createNameFromMasks(masks) + _lodash.uniqueId();
    ...
    this.otherwise = function (fn) {
        return atlant.streams.reg.call(this, atlantState.devStreams.otherwiseStream, fn, true);
    };

    // Creates stream which will be called when render error is happend
    this.error = function (fn) {
        return atlant.streams.reg.call(this, atlantState.devStreams.errorStream, fn, true);
    };
    ...
    this.action = function (action, fn) {
        return atlant.streams.reg.call(this, action, fn, true);
    };
    ...
    this.interceptor = function (fn) {
        var interceptorName = _lodash.uniqueId();atlantState.interceptors.push(interceptorName);return atlant.streams.reg.call(this, interceptorName, fn, false);
    };
    ...
    this.data = Object.defineProperties({}, {
        routes: {
            get: function get() {
                return lodash(atlantState.routes).uniq() // @TODO better not to double it for info :) 
                .value();
            },
            configurable: true,
            enumerable: true
    ...
    // Create stream.
    this.stream = function (bus) {
        return new _incStream.Stream(atlantState, prefs, bus);
    };

    this.streams = {
        get: function get(name) {
            // @deprecated, used here only because there are many actions with Bacon.Bus()'es declared yet.
            if (!atlantState.busses[name]) {
                var bus = _incBaseStreams2['default'].bus();
                _this.streams.put(name, bus);
            }

            return atlantState.busses[name];
        },
        put: function put(name, bus) {
            // @deprecated because sets Bacon.Bus() into atlant streams cache. Bacon.Bus() should be avoided.
        ...
        reg: function reg(busOrName, fn, canBeIntercepted) {
            if ('string' !== typeof busOrName && !(busOrName instanceof Bacon.Bus)) throw new Error('Provide either Bacon.Bus() either Stream name.');

            if (!busOrName) {
                _utilsLog2['default'].warn('Failed stream source:', fn);throw new Error('Atlant.js Bacon.Bus() stream or stream name is not provided!');
            };
            if (!fn || 'function' !== typeof fn) {
                _utilsLog2['default'].warn('Failed stream source:', fn);throw new Error('Atlant.js: follow stream function is not provided!');
            };

            var busName = 'string' === typeof busOrName ? busOrName : _lodash.uniqueId();
            if (busName in atlantState.busses) {
                _utilsLog2['default'].warn('source:', fn);throw new Error('Several actions with 1 name is not supported now. ' + busName + ' is not unique.');
            }

            if ('string' === typeof busOrName) {
                this.streams.get(busName); // creating new bus and stream
            } else {
                    // Bacon.Bus() was passed
                    this.streams.put(busName, busOrName); // reusing passed bus
                }

            atlantState.finishes[busName] = _incBaseStreams2['default'].bus();
            ...
            atlantState.busses[busName].onValue(function (depValue) {
                ...
                if (stream instanceof _incStream.Stream) {
                    _utilsLog2['default'].warn('Failed stream source:', fn);throw new Error('You should end the Stream. Try add more .end()\'s ');
                };
                if (!stream || !(stream instanceof _incStream.ReadyStream) && !(stream instanceof Bacon.Bus)) {
                    _utilsLog2['default'].warn('Failed stream source:', fn);throw new Error('Unknown return from Stream function');
                };
                ...
                stream.onValue(function (_) {
                    return atlantState.finishes[busName].push(_);
                });
...
exports.__esModule = true;
exports.ReadyStream = ReadyStream;
...
function ReadyStream(streamState, bus, stream) {
    var _this = this;

    if (stream instanceof Stream) Object.keys(stream).forEach(function (__) {
        return _this[__] = function (_) {
            return _utilsLog2['default'].error('This stream is ready! You can only .push(data) or subscribe: .onValue(), onStart(). The "' + __ + '" is wrong here!');
        };
    });
    ...
    this.push = function () {
        ...
        return setTimeout(function (_) {
            return bus.push.apply(bus, args);
        }, 0);
    };
    this.pushSync = function (_) {
        return bus.push(_);
    };

    Object.defineProperty(this, 'canBeIntercepted', { get: function get() {
            return streamState.canBeIntercepted;
        }, set: function set(value) {
            streamState.canBeIntercepted = value;_utilsLog2['default'].log('someone set canBeIntercepted:', streamState.canBeIntercepted);
        } });

    this.onStart = function (_) {
        bus.onValue(_);
    };

    this.onValue = function (_) {
        // Stream is Resolved
        if (!streamState || !streamState.streamCallbacks) _utilsLog2['default'].log('not supported yet, use named action instead of Bacon.Bus()');else streamState.streamCallbacks.push(_);
    };

    return this;
}

function Stream(atlantState, prefs, fn) {
    var _this2 = this;
            ...
            atlantState.viewSubscriptionsUnsubscribe[viewName] = atlantState.viewSubscriptions[viewName].onValue((function (upstream, viewName, scope, doRenderIntoView, value) {
                ...
                if (!_.isEqual(data, atlantState.viewData[viewName])) {
                    ...
                    if (streamState.resolveWhen && streamState.resolveWhen(data) && streamCallbacks.length) {
                        streamState.streamCallbacks.forEach(function (_) {
                            return _(data);
                        });
                    }
                }
            ...
            try {
                ...
                if (types.RenderOperation.redirect === upstream.render.renderOperation) {
                    ...
                    var renderResult = doRenderIntoView(scope).then(function () {
                        ...
                        atlantState.devStreams.errorStream.push();return Bacon.End();
                    });
            ...
            if ('function' !== typeof dep) {
                ...
                stream = stream.flatMap((function (store, depName, dep, isAtom, upstream) {
                    ...
                    return treatDep(dep)(atomValue).mapError(function (_) {
                        ...
                        if (upstream.ref == 'something') _utilsLog2['default'].log('values:', streamState.canBeIntercepted && s.isObject(results) && 'status' in results);
                        ...
                        if (streamState.canBeIntercepted && s.isObject(results) && 'status' in results) {
                            ...
                            var _ret = (function () {
                                ...
                                res = finish.take(1).flatMap(function (_) {
                                    return _;
                                });
                                counter = baseStreams.bus();
                                scan = counter.scan(atlantState.interceptors.length, function (a, b) {
                                    return a - b;
                                });

                                scan.onValue(function (_) {
                                    _utilsLog2['default'].log('scan interceptor:', _, depName);if (_ === 0) {
                                        finish.push(results);
                                    }
                                });
                                ...
                                atlantState.interceptors.forEach(function (name) {
                                    ...
                                    var stream = atlantState.finishes[name].map(function (_) {
                                        return _.then(function (_) {
                                            ...
                                            _utilsLog2['default'].log('end interceptor', _, depName);
                                ...
                                _utilsLog2['default'].log('the res:', res, depName);
                        ...
                        _utilsLog2['default'].log('results:', results, depName);
                        if (!upstream.depends) upstream.depends = {};
    ...
    var _inject = function _inject(key, expression) {
        ...
        _utilsLog2['default'].warn('Use of atlant.inject( ' + key + ', ' + expression + ' ) is deprecated at ' + atlantState.lastMask);
    ...
    var _join = function _join(key, expression) {
        ...
        _utilsLog2['default'].warn('Use of atlant.join( ' + key + ', ' + expression + ' ) is deprecated at ' + atlantState.lastMask);
    ...
    var closeBlock = function closeBlock(renderOperation, viewName) {
        ...
        if (void 0 !== State.state.lastIf) {
            ...
            return new ReadyStream(streamState, root, _this2);
        }
    };
    ...
    var _render = (function () {
        ...
        return function (renderProvider, viewName, once, renderOperation) {
            ...
            if (renderOperation === types.RenderOperation.nope) {
                // Just close if's and stream if "nope"
    ...
    var _reject = function _reject() {
        State.state.lastOp = State.state.lastOp.map(function (_) {
            streamState.streamCallbacks.forEach(function (callback) {
                return callback(Promise.reject(_));
            });
            return _;
        });
    ...
    var _resolve = function _resolve() {
        State.state.lastOp = State.state.lastOp.map(function (_) {
            streamState.streamCallbacks.forEach(function (callback) {
                return callback(Promise.resolve(_));
            });
            return _;
        });
...
},{"inc/base-streams":4,"inc/clientFuncs":5,"inc/interfaces":6,"inc/performance":7,"inc/state":8,"inc/types":12,"utils/lib":16,"utils/log":17,"utils/utils":18,"views/views":19}],11:[function(require,module,exports){
"use strict";
        ...
        var update = function update() {
            try {
                ...
                if (root) {
                    ...
                    _this.attach('root', selectors['root']);
                }
                error = true;
            }
        };
