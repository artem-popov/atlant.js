...
function Atlant() {
    ...
    var assignRenders = (function () {
        ...
        var subscribeView = function subscribeView(viewName, doRenderIntoView, scope, upstream) {
            ...
            viewSubscriptionsUnsubscribe[viewName] = viewSubscriptions[viewName].onValue((function (upstream, viewName, scope, doRenderIntoView, value) {
                ...
                value = _(upstream.chains).map(function (o) {
                    return _(o).map(function (_) {
                        return _;
                    }).flatten().value();
                }).flatten().reduce(function (acc, i) {
            ...
            .map((function (depName, store, dep, upstream) {
                ...
                if ('undefined' !== typeof upstream.ref && 'undefined' !== typeof store) {
                    ...
                    (function () {
                        ...
                        if (store.dependsOn && '' !== store.dependsOn) console.log('select depending ', upstream.ref, 'on', store.dependsOn);
                        if (store.dependsOn && '' === store.dependsOn) console.log('select', upstream.ref, 'is independent');
                        if ('' === store.dependsOn) console.log('select', 'no dependency');
                        ...
                        var f = dependence ? f = function (_) {
                            return getValue(dependence(_));
                        } : getValue;

                        var chainKey = dependence ? upstream.dependences[store.dependsOn] : upstream.ref;
        ...
        return function (masks, matchingBehaviour, whenType) {
            ...
            State.state.lastWhen = rootStream.map((function (masks, upstream) {
                ...
                if (scrollToTop.value && 'undefined' !== typeof window) {
                    setTimeout(function () {
                        console.log('Atlant.js: scrolling 1');prefs.scrollElement().scrollTop = 0;
                    }, 0);
                }
    ...
    var _action = function _action(action, isAction, depCode) {
        ...
        State.state.lastWhen = action.map((function (depName, injects, nameContainer, stats, whenId, isAction, depValue) {
            ...
            if (scrollToTop.value && 'undefined' !== typeof window) {
                setTimeout(function () {
                    console.log('Atlant.js: scrolling 2');prefs.scrollElement().scrollTop = 0;
                }, 0);
            }
