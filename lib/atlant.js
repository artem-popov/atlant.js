...
function AtlantStream(name, atlantState) {
    ...
    var worker = function worker(depValue) {
        ...
        userStream.then(function (_) {
            _utilsLog2['default'].log('action(resolved):', _);resolve();
        });
        _utilsLog2['default'].log('depValue:', name, depValue, atlantState.whenData);
    ...
    var push = function push(isSync, args) {
        ...
        _utilsLog2['default'].log('push stream args:', name, args);
        ...
        if (!_this.isAttached()) {
            ...
            _utilsLog2['default'].log('Detected push to action:', name, fn);pusher();
        }
    ...
    var pushBus = function pushBus(isSync, args) {
        _utilsLog2['default'].log('push bus args:', name, args);
    ...
    this.attach = function (_) {
        if (!_this.isAttached() && _ && typeof _ === 'function') {
            fn = _;
            _utilsLog2['default'].log('And finally it get name!:', name, 'fn', fn);
