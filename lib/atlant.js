...
function Atlant() {
    ...
    var assignRenders = (function () {
        ...
        var subscribeView = function subscribeView(viewName, doRenderIntoView, scope, upstream) {
            ...
            var lastAtom = upstream.atoms.length ? upstream.atoms[upstream.atoms.length - 1] : void 0;
        ...
        var createDepStream = function createDepStream(stream, depName, dep, injects, store, isAtom) {
            ...
            .map((function (depName, store, dep, upstream) {
                ...
                if ('undefined' !== typeof upstream.ref && 'undefined' !== typeof store) {
                    ...
                    var bus = store.storeName in upstream.lastParent ? upstream.lastParent[store.storeName].bus : store.bus;

                    if (lastAtom) {
                        bus = bus.merge(lastAtom.bus).scan({}, function (x, y) {
                            return s.copy(_.extend({}, x, y));
                        }).toEventStream();
                    }

                    bus = bus.map(getValue).skipDuplicates(_.isEqual); // Real works
    ...
    var _select = function _select(isAtom, dependsBehaviour, partName, storeName, dependsOn) {
        ...
        if (dependsOn && (!isArray(dependsOn) && 'string' !== typeof dependsOn || isArray(dependsOn) && dependsOn.filter(function (_) {
            return 'string' !== typeof _;
        }).length)) throw new Error('atlant.js: dependsOn param should be either a string or array of strings');
