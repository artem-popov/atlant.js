...
function Atlant() {
            ...
            baseStreams.onValue(theStream, function (upstream) {
                ...
                try {
                    ...
                    if (types.RenderOperation.redirect === upstream.render.renderOperation) {
                        ...
                        var putInfo = function putInfo(atom, atomValue) {
    ...
    var _depends = (function () {
        ...
        var createDepStream = function createDepStream(stream, depName, dep, injects, store, isAtom) {
            ...
            if ('function' !== typeof dep) {
                ...
                stream = stream.flatMap((function (store, depName, dep, isAtom, upstream) {
                    ...
                    var atomParams = (function (scope, where, updates) {
                        return where(_.merge({}, scope, updates));
            ...
            .map((function (depName, store, dep, upstream) {
                // upstream.dependNames store name of all dependencies stored in upstream.
                ...
                if ('undefined' !== typeof upstream.ref && 'undefined' !== typeof store) {
                    ...
                    var getValue = (function (ref, atomParams, u) {
                        ...
                        var res = dep()(params);
                        var result = _defineProperty({}, ref, res); // actually used dep, which has side-effect
                        return _.extend(u, result);
                    }).bind(void 0, upstream.ref, upstream.atomParams);

                    var bus = store.bus;

                    if (upstream.lastAtom) bus = bus.merge(upstream.lastAtom.bus) // Depending on one upper bus
                    .scan({}, function (x, y) {
                        return _.extend(x, y);
                    }).toEventStream();

                    bus = bus.map(getValue);
