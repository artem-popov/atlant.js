...
function Atlant() {
    ...
    atlantState.rootStream.onValue(s.tryD(function (upstream) {
        ...
        _whens.items.forEach(function (whenData) {
            ...
            var stream = whenData.route.fn instanceof _incStream.Stream ? whenData.route.fn : whenData.route.fn(depData); // @TODO should be a Stream.
    ...
    var _interceptor = function _interceptor() {
        ...
        State.state.lastWhen = atlantState.devStreams.interceptorBus.map((function (depName, injects, whenId, obj) {
    ...
    this.otherwise = function (fn) {
        return atlant.streams.reg.call(this, atlantState.devStreams.otherwiseStream, fn);
    };

    // Creates stream which will be called when render error is happend
    this.error = function (fn) {
        return atlant.streams.reg.call(this, atlantState.devStreams.errorStream, fn);
    };
    ...
    this.action = function (action, fn) {
        return atlant.streams.reg.call(this, action, fn);
    };
    ...
    this.streams = {
        ...
        reg: function reg(busOrName, fn) {
            ...
            atlantState.busses[busName].onValue(function (depValue) {
                ...
                _utilsLog2['default'].log('will push to ', busName, ' value: ', depValue);
...
function Stream(atlantState, prefs, fn) {
    ...
    var renderView = (function () {
        ...
        var renderIntoView = function renderIntoView(viewProvider, upstream, viewName, render, scope) {
            ...
            return renderD(viewProvider, upstream, atlantState.activeStreamId, viewName, scope, atlantState.devStreams.errorStream).then(function (_) {
                // @TODO make it better
...
var Render = function Render(React) {
    ...
    this.render = function (viewProvider, upstream, activeStreamId, name, scope, errorStream) {
        ...
        if (instance && instance.isMounted && instance.isMounted() && instance.forceUpdate) {
            try {
                ...
                _utilsLog2['default'].error(e.stack);errorStream.push(e);
            }
        }
    ...
    this.clear = function (viewProvider, upstream, activeStreamId, name, scope, errorStream) {
        ...
        }, upstream, activeStreamId, name, scope, errorStream);
    };
