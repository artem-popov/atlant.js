...
function Atlant() {
    ...
    atlantState.rootStream.onValue(s.tryD(function (upstream) {
        ...
        _whens.items.forEach(function (whenData) {
            ...
            if (!stream || !(stream instanceof _incStream.ReadyStream)) {
                _utilsLog2['default'].warn('Failed stream source:', whenData.route.fn);throw new Error('Unknown return from Stream function', whenData.route.fn);
            };

            if (whenData.when.type === types.WhenOrMatch.when) stream.onResolve(function (_) {
...
function Stream(atlantState, prefs, fn) {
    ...
    var closeBlock = function closeBlock(renderOperation, viewName) {
        ...
        if (void 0 !== State.state.lastIf) {
            ...
            _utilsLog2['default'].log('closeBlock:if');

            return _this2;
        } else {
            _utilsLog2['default'].log('closeBlock:else');
    ...
    var _render = (function () {
        ...
        return function (renderProvider, viewName, once, renderOperation) {
            ...
            _utilsLog2['default'].log('attaching render:', viewName, renderOperation);
            if (renderOperation === types.RenderOperation.nope) {
                _utilsLog2['default'].log('will close');var that = closeThisBlock();State.state.lastOp.onValue(function (_) {
                    return _;
                });return that;
            ...
            _utilsLog2['default'].log('will close block');
