...
function Atlant() {
    ...
    this.streams = {
        ...
        reg: function reg(busOrName, fn, canBeIntercepted) {
            if ('string' !== typeof busOrName && !(busOrName instanceof Bacon.Bus)) {
                _utilsLog2['default'].warn('Failed stream source:', fn);
                throw new Error('Provide either Bacon.Bus() either Stream name.');
            }
            ...
            if ('string' === typeof busOrName && busName in atlantState.busses && busName in atlantState.fns && atlantState.fns[busName]) {
