...
function Atlant() {
    ...
    atlantState.rootStream.onValue(s.tryD(function (upstream) {
        ...
        _whens.items.forEach(function (whenData) {
            ...
            _utilsLog2['default'].log('when:', whenData, stream, whenData.route.fn);
            ...
            _utilsLog2['default'].log('When: this is before pushing into received stream.', stream);
    ...
    this.streams = {
        get: function get(name) {
            if (!atlantState.streams[name]) {
                ...
                atlantState.streams[name] = _this.stream(bus);
            }

            return atlantState.streams[name];
        },
        reg: function reg(stream, fn) {
            ...
            atlantState.busses[streamName].onValue(function (depValue) {
                ...
                var stream = fn instanceof _incStream.Stream ? fn : fn(depValue);

                if (stream instanceof _incStream.Stream) stream.push(depValue);else _utilsLog2['default'].error('unknown return from Stream function');
            });
...
function ReadyStream(streamState, stream) {
    var _this = this;
    ...
    this.push = function () {
        ...
        return setTimeout(function (_) {
            return stream.push.apply(stream, args);
        }, 0);
    };
...
function Stream(atlantState, prefs, fn, bus) {
    var _this2 = this;

    if (bus && bus instanceof Bacon.Bus) {
        _utilsLog2['default'].warn('1Using Bacon.Bus streams are deprecated!');
    ...
    var closeBlock = function closeBlock(renderOperation, viewName) {
        ...
        if (void 0 !== State.state.lastIf) {
            ...
            return new ReadyStream(streamState, _this2);
        }
    };
